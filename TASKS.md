...
# ▶ REC-FMEA — THE GATED REPAIR OF `src/FF_Recovery.cpp`

> ↗ **The plan of record is `../FastFHIR-benchmark/recovery_handoff.md`** in the
> COMPANION REPO. It holds the Failure Mode and Effect Analysis (31 modes, organised
> by pipeline stage, each with cause, effect, detection gap, severity, occurrence and
> evidence status) and seven work packages. Cite modes by their F-identifier: they are
> the shared vocabulary across that document, this file and the test suite.

**WP1 is DONE (2026-09-22).** The gates live in `tests/cpp/test_recovery.cpp` and assert
two things over every eligible structural byte of two fixtures, rather than over a
hand-picked edge:

1. `popcount(introduced) == 0`, where `introduced = (clean XOR repaired) AND NOT (clean
   XOR damaged)`. A repair may only move a damaged bit back toward clean.
2. Either the repair is exact, or the report SAYS SO — every flipped byte still
   differing from clean carries an Ambiguous or Unrecovered verdict naming its edge, or
   a header verdict naming its field. "No verdict" and "no damage" must not look alike.

Both oracles enumerate their fixtures exhaustively (3,712 and 3,072 probes) and the
whole binary runs in 1.6 s. The measured red list, with the cluster-to-mode assignment,
is the block comment above `gate_set_options()` in the test file; do not edit it without
the commit that changes a number in it.

**ctest registration is deliberate.** `cpp_ff_test_recovery` runs the 251 legacy checks
with `--gates off`, so one red gate cannot drown them. Each gate then has its own entry,
and the three that are red today carry `WILL_FAIL TRUE` — which means the moment a work
package fixes one, its entry FLIPS TO FAILING and whoever fixed it must clear the
property and update the red list. A known-failing test nobody is forced to revisit is
how a suite starts lying.

```bash
./build/ff_test_recovery --gates only --filter single_flip_oracle
```

Three modes came out of the gates that neither the analysis nor the earlier review had
found. All three are in the handoff's §5:

- **F30**, the largest cluster (218 of 3,712 probes): `apply`'s TagRepaired arm declines
  to write when `plausible_tag` says the child could be an innocent block, but
  `classify_one` never applies that guard — so the verdict stays a confident
  `TagRepaired`, `rep.tag_repaired` counts it, and the damage stays on the wire while
  the report claims a repair. The guard is right; its LOCATION is wrong.
- **F29**, the only cluster that writes a wrong byte: `reconcile_header`'s
  `reconcile_singleton` has no flip-budget check, unlike the root reconciliation right
  below it. The singleton tags are `0x0004`/`0x0005`/`0x0006`, so one flipped tag byte
  anywhere fabricates a URL directory, and a `URL_DIR_OFFSET` that was `FF_NULL_OFFSET`
  becomes a pointer — around 60 invented bits.
- **F31**: `FHIR_VERSION_R4` (`0x0400`) and `FHIR_VERSION_R5` (`0x0500`) are **one bit
  apart**, so a flip lands exactly on the other legal revision, the closed-set check
  scores it at cost 0 and reports Intact. Exactly one of the 432 header bits.
...
| Date | Item | Outcome |
|---|---|---|
| 2026-09-22 | **Conformance API restyle, and the consumer entry point** | Two things, one seam. **(1) The FF_ boundary.** `Builder_t::attach_layer` (still `void`, still throws on an ABI mismatch) gains a wrapper `FF_BuilderAttachLayer(FF_BuilderAttachLayerInfo) -> FF_Result`, so the throw becomes a Result at the API edge; the hook ABI (`CheckFn`/`StreamCheckFn`/`Status`/`Entry`/`ValidationHooks`) keeps its trivially-copyable shape because it crosses a library seam and fires before `claim_space`. **`FF_` means GLOBAL here**: `FF_BuilderAttachLayer`/`FF_BuilderAttachLayerInfo` live at global scope and keep the prefix, while everything inside `namespace FastFHIR::Conformance` drops it — `conformance_layer()`/`conformance_rules(tag)` keep their original names and the bundled hook argument is `StreamCheckInfo`. An interim pass had prefixed all three; Ryan reversed it (2026-09-22) on the rule that the prefix marks global scope and is redundant behind two namespaces, and extended the same rule to the ABI constants, which are now `CONFORMANCE_ABI` and `CONF_VERSION_R4`/`R5`/`ALL`. **The rule for the whole tree: `FF_` marks a GLOBAL name.** A name already reached through `FastFHIR::` or `FastFHIR::Conformance::` does not carry it, whatever its case — which is the same split `FF_DateTimeParts` (global carrier) and `FastFHIR::DateTime` (namespaced value type) already follow. The six-positional-value `StreamCheckFn` bundles into that one `StreamCheckInfo` (CONTRIBUTING §5), which `dispatch_stream` stamps with a per-layer `self`. **(2) The consumer half.** `FastFHIR::Conformance::validate_stream(const Memory&)` (`include/FF_Validate.hpp`, `src/conformance/FF_Validate.cpp`) conformance-checks a FINISHED stream: the arena, its extent, the revision and the root are read back out of the sealed `FF_HEADER` rather than demanded from the caller, and it passes the **mapped extent, never `capacity()`** — the Builder passes capacity because it is writing into that reservation, and a reader that did would let every bounds check pass on addresses the stream does not own. A second overload takes a caller-configured layer, which is how `Report` policy and a sink collect every finding instead of only the first. `StreamCheckInfo` and `dispatch_stream` moved OUT of the installed `FF_Conformance.hpp` into the never-installed `FF_Conformance_internal.hpp`; the public header declares the struct **incomplete**, which is enough for `StreamCheckFn` and keeps the standalone-compile property (verified). `install_smoke` asserts the internal header is absent from the package and `FF_Validate.hpp` ships with `fastfhir_conformance`. **`dispatch_stream` is NOT `noexcept`** — a `StreamCheckFn` may allocate (the shipped one builds a `BundleIndex`), so `noexcept` would turn a recoverable `std::bad_alloc` into `std::terminate`; an interim pass had marked it `noexcept` and it was removed. `BUILD.bazel` was missing `FF_StreamCheck.cpp` entirely, so a Bazel build linked a layer with no stream check and answered OK to every reference question; both it and `FF_Validate.cpp` are in the target now. `ff_test_conformance` drives the FF_ boundary and the read path (**23 cases / 137 checks**), including the same unresolved `urn:` written to disk with NO layer attached and then caught by `validate_stream` on a read-only mount. ctest 66/66, pytest 63. |
| 2026-09-22 | **Identity slot P3 — the kind and its value types** | `FF_FIELD_ID` is added to `FF_FieldKind` (value 15, appended after `FF_FIELD_URL`) with `ff_slot_width` returning **16**, and the identity value types land beside `FF_DateTimeParts`: `FF_UUID` (sixteen raw `BYTE`s, all-ones default, a hex constructor that accepts only the canonical lowercase 8-4-4-4-12 spelling with or without a `urn:uuid:` prefix and throws otherwise) and the form-tagged `FF_Id` (ABSENT, UUID, GENERATED, INTERNED, RAW_STRING, plus the build-time-only POINTER/PENDING). `FF_Id::read_slot` is the ONE place the three ordered tests of §17.2 are written — all-ones → ABSENT (tested FIRST, or an all-ones slot reads as a UUID with version nibble 15), check byte `0x01` AND a zero version byte → a pointer form, otherwise sixteen inline UUID bytes — and `write_slot` encodes them back. `FF_Id::generated(off)` then `offset(id)` returns `off`; every other form, **including RAW_STRING whose payload IS a plausible offset**, yields `FF_NULL_OFFSET` (all-ones, never a real block, so the wrong answer degrades instead of resolving). **No field uses the kind, so no wire byte changes.** Adding the enum triaged every `-Wswitch` site the compiler enumerated (`ff_kind_is_inline_scalar`, `ff_kind_name`, the recovery-reference switch) and `FF_IsFieldEmpty` gained the 16-byte all-ones case (else every identity field would report absent and drop on export — the trap `FF_FIELD_DATETIME`/`FF_FIELD_INT32` fell into). New `ff_test_identity` (12 cases / 78 checks); mutation-tested three ways (all-ones check removed → 4 failures; `offset()` widened to RAW_STRING → 1; the pointer test's version-byte half dropped → 1). ctest 66/66, pytest 63. |
| 2026-09-22 | **Identity slot P2 — directory hardening (Finding 3)** | The `FF_URL_DIRECTORY` read path and the validator were hardened against a forged table, the class Finding 3 named. `FF_URL_DIRECTORY::seg_string` and `get_url` no longer index off a stream-supplied count: both clamp the declared `entry_count` to what the stream can physically hold (`(size − table) / URL_ENTRY_SIZE`) and refuse an index past that, and `get_url`'s prior-chain walk is **cycle-guarded** — a chain has at most one link per entry, so a longer walk is a forged `prior_idx` closing a loop and now returns empty instead of spinning forever. The directory hangs off the HEADER, so the root walk never reached it: the deep validator gained `walk_directory`, called by BOTH `validate_FFHR_stream` and `validate_FFHR_stream_deep` (rootless streams included), which bounds the count, checks each `prior_idx` against the table, and requires every segment offset to land on a block that vouches for itself — the same witness `walk()` applies to every other block. Both `FF_FIELD_URL` print sites were merged into one `print_url_json`, so an unresolvable ref prints `""`, never `null`. `ff_test_graph_bounds` section 8 drives all of it (forged count, forged `prior_idx`, segment offset 0, a clobbered segment witness, the read-path degradation, and a self-referential chain that must terminate); mutation-tested (get_url bound dropped → 1 failure; cycle cap removed → hang; validator visit removed → 8 failures). ctest 65/65, pytest 63. |
| 2026-09-22 | **Identity slot P1 — directory write path (URL-1 extended)** | The `FF_URL_DIRECTORY` block is now written at **finalize**, not up front by `FF_PredigestExtensionURLs` (its Phase 5). Laid down once when the entry set is complete, the table is no longer fixed-size from the first intern, so it can GROW — which fixes Finding 1's live defect: `Extension.url` is usable from the direct POCO path (`builder->intern_url(url)` → index), and a re-opened Builder can register new URLs. The radix trie and entry list moved out of the ingestor into **`FastFHIR::UrlDirectory`** (`include/FF_UrlDirectory.hpp` + `src/FF_UrlDirectory.cpp`, internal — pimpl-owned by the Builder, not installed); the ingestor keeps only the producer/consumer funnel and calls `Builder_t::intern_url`. Only the header-and-entry-table block moved — the segment `FF_STRING`s are still claimed during `intern()`. Re-open now `load()`s the existing table into the intern trie so its URLs survive and more can be added. No wire change: the block's bytes and layout are unchanged, only WHEN it is written. `ff_test_file_modes` gains `url_grow` (a re-opened Builder interns a URL the first build never saw; both survive reseal) and `url_directory`'s offset assertion was re-pointed at the property it protected (the loaded table) rather than the mechanism; mutation-tested (no write → 2 failures, no load → 5). ctest 65/65, pytest 63. |
| 2026-09-22 | **T10 — finalize-time reference check (Block K)** | The conformance layer gained its first WHOLE-DOCUMENT check, and the boundary gained the hook to carry it: `ValidationHooks::stream_check`, `dispatch_stream()`, and `Check::UNRESOLVED_REFERENCE`. `CONFORMANCE_ABI` stays 1: the struct grew, but the version only moves when a layer built by another toolchain must agree on it, and there is no such layer yet. `Builder::finalize` runs it after the mutators drain and before `seal_stream`, so the entry set is complete — the question "does every reference resolve" cannot be asked earlier, because the target may not be written yet. Classification is by reference FORM: `urn:` unresolved is an error, a relative `Type/id` unresolved is a warning, an absolute URL and an identifier-only `Reference` are not this check's business. A refusal releases the finalize latch (the check reads and never writes, so a caller can correct the stream and finalize again) unless the policy is Report, which counts and writes on. `src/conformance/FF_StreamCheck.{hpp,cpp}` walks the document the way `print_json` does — same field table, same slot lookup — so a reference is found at any depth, singular or repeating; it reuses T11's `BundleIndex` for resolution and `Conformance::report()` for the sink and counter. **Byte identity holds**: a stream finalized with the check attached is byte-identical to one without it, including on the reported-failure path. `ff_test_conformance` 12 cases / 44 checks → **18 cases / 99 checks** (resolves, urn error under Throw, urn reported under Report + byte identity, relative warns without stopping even under Throw, absolute silent, and a refusal leaving the Builder still mutable); mutation-tested with three deliberate breaks (urn never hard, walk removed, latch never released — 7, 11 and 1 failures respectively). A new generator gate asserts the layer WIRES the hook, so a layer that attaches but checks nothing fails the build. |
