...
| **P0** | **REC-18 (planned)** | **Gaps** — tile the arena by block extent; a hole is a block with *neither* witness left, the one damage class scan+reachability cannot represent | `^## REC-18` |
...
## REC-18 — GAPS: find the blocks that have no surviving witness at all

**Ryan, 2026-08-27. Plan only — do not start implementing until this section is read.**

### Why this exists: the case the current machinery cannot represent

`recover()` derives orphans as **self-consistent ∧ ¬reachable**. That set can only ever
contain blocks `scan()` found, and `scan()` finds a block by `LOAD_U64(o) == o`. So:

| damage | `scan()` sees it? | `walk_chain()` reaches it? | in the orphan set? |
|---|---|---|---|
| parent offset flipped, child intact | yes | no | **yes** — mode 1, already handled |
| child `VALIDATION` flipped, parent intact | **no** | yes (parent names it) | no — but the parent still points there, so `recover()` finds it as `PositionRepaired` |
| **child `VALIDATION` flipped AND the parent reference broken** | **no** | **no** | **NO — invisible** |

That third row is the target. Both witnesses are gone, so the block cannot be found by
following anything or by scanning for anything. **It can only be found by absence**: it
still occupies bytes, and if every other block's extent is known, those bytes are a hole.

This is the two-corruption boundary P0-3 names and the recovery test currently documents as
residual. Gaps do not close it completely — a hole says *something of size N was here*, not
what — but they turn "invisible" into "located and sized", which is what the Hamming ranker
needs to have a candidate position at all.

### Feasibility — measured 2026-08-27, not assumed

Probe over a real 3.3 MB Synthea stream (60,664 map entries) with extents supplied for
every entry:

```
map entries=60664   stream=3330184 bytes   entries with no derivable size=0
gaps=1 totalling 352 bytes (0.01%)         overlaps=0
```

**The arena tiles.** The single residual gap was the URL directory's entry payload, which
the probe charged header-only — size it and the tiling is exact. That is the property the
whole feature rests on, and it holds.

### The three extents that must exist first

`classify_block()` already sizes `Array`, `String` and `Header`. Everything else is `size = 0`.

1. **Generated blocks — derive from the compiled reflection table.** No generator change:
   `max(field_offset + ff_slot_width(kind))` over `reflected_fields_view(tag)`, floored at
   `DATA_BLOCK::HEADER_SIZE`. `ff_slot_width()` (`include/FF_Primitives.hpp:722`) is
   `constexpr` and total. Verified exact against the compiled constants:
   `FF_OBSERVATION` 288, `FF_PATIENT` 191, `FF_CODING` 55.
2. **Three hand-written primitive tags have no generated reflection.** Measured counts on
   one bundle: `RECOVER_FF_CODEABLE_CONCEPT` (0x0009) ×6,593,
   `RECOVER_FF_URL_DIRECTORY` (0x0005) ×1, `RECOVER_FF_CHECKSUM` (0x0004) ×1. Each has a
   compiled `HEADER_SIZE`; `FF_CODEABLE_CONCEPT` is `HEADER_SIZE(12) + LENGTH`, and the URL
   directory must include its **entry payload**, not just its header. A small switch
   beside the existing `FF_STRING` / `FF_ARRAY` cases covers all three.
3. **Containment must not double-count — this is the judgment call.** Charging every array
   its full `HEADER_SIZE + count × stride` produced **11,374 overlaps totalling 54 MB in a
   3.3 MB stream**. Not corruption: an inline array of self-describing block headers has
   each element in the map *already*, so the array's extent encloses entries that also
   claim those bytes.

   > **Rule.** An entry is charged only the bytes **no other entry owns**. Arrays whose
   > elements are themselves map entries (`INLINE_BLOCK` + a non-scalar, non-resource
   > element tag → `ElementShape::InlineBlock`) are charged **header-only**. Every other
   > array — raw scalars, resource tuples, and the string OFFSET table — is charged its
   > full extent, because those bytes belong to nothing else. This is the same
   > `element_shape_of()` distinction as the `RECOVER_FF_RESOURCE` fix; reuse it, do not
   > re-derive it.

### Version skew — an old parser on a newer stream (Ryan)

A future engine appends slots to a block's V-Table, so its `HEADER_SIZE` grows. An **old**
parser derives the extent from **its own** reflection table, which is shorter, so it under-
sizes every block of that tag and sees a small trailing gap after each one. **These are
benign and must never be reported as damage.**

They are distinguishable, and the discriminators compose — apply in this order:

1. **Version gate (cheapest).** `FF_HEADER::VERSION` carries the encoded engine version;
   decode with `FF_HEADER_ENGINE_VERSION()`. If the stream's engine version is **not newer**
   than the reader's, skew is impossible and every gap is suspect. Note `Parser` reads this
   field and never rejects on it (CLAUDE.md) — this is the first use that acts on it.
2. **Systematicity.** A version gap trails *every* instance of a tag at the *same* size; a
   damage hole is a one-off. Per tag, collect the trailing gap size; where the instances
   agree, record it as that tag's version delta and reclassify those gaps as `VersionSkew`.
   **This is self-calibrating** — it needs no version table and no negotiation, which
   matters because the reader by definition does not know the newer layout.
3. **Position.** A version gap begins exactly at `block_offset + derived_HEADER_SIZE` — a
   V-Table boundary. A damage hole begins wherever the previous entry ended.
4. **Size floor.** A hole must be at least `DATA_BLOCK::HEADER_SIZE`; anything smaller
   cannot be a block. Weak on its own (a large version delta can exceed a small block), so
   never use it alone.

Thin-sample caveat: a tag with one or two instances gives no statistic. Under a version
mismatch, fall back to treating a small V-Table-boundary trailing gap as skew and say so in
the report; under a version match, do not.

**This is forward-looking, not urgent.** Pre-alpha allows breaking the wire format, and no
stream with a different engine version exists yet. Build the classification now because it
is cheap now and unreachable later — but do not gate REC-18 on a version negotiation
design.

### Scope limits — state them, do not silently assume them

- **Compact archives are out of scope.** `Recovery` does not consider them at all today
  (`grep -n compact src/FF_Recovery.cpp` → nothing), and the compact layout is a
  presence-bitmask rewrite with different geometry. The layout mode is in the header's high
  version bits (`FF_STREAM_LAYOUT_MASK`); **read it and refuse gap analysis on a compact
  stream** rather than producing nonsense.
- **Beyond the write head is not a gap.** `StreamMap::file_size` already carries the bound.
- **A gap is not an orphan.** `recover()` already uses "orphan" for *self-consistent but
  unreferenced*. A gap is *unattributed bytes*. Different evidence, different repair,
  different report field. Keep the two names apart.
...
- [x] **REC-18.3 — `Gap` and the sweep.** ✅ **DONE 2026-08-27.** Linear pass over the ordered map in offset order:
  ...
  struct Gap {
      ...
      Size         length = 0;
      RECOVERY_TAG after  = FF_RECOVER_UNDEFINED;  // tag of the block the gap trails
      GapClass     class_ = GapClass::Hole;        // Hole | VersionSkew | Trailing
  };
  ```
...
Extend `tests/cpp/test_recovery.cpp`. Every case must be **red before, green after**:

1. **Clean stream ⇒ zero gaps, zero overlaps, zero unsized entries.** Assert a non-zero
   entry count *first*, or the whole thing passes vacuously.
2. **Break only a child's `VALIDATION`** ⇒ `scan()` loses it, a hole appears at its offset,
   and the hole's length equals that tag's derived `HEADER_SIZE`.
3. **Break the `VALIDATION` *and* the parent reference** — the REC-18 target. Assert the
   block is reported as a `Hole` and is **not** silently absent. This case must fail before
   the change, which is the point of writing it first.
4. **Simulated version skew:** widen one tag's extent by hand (or shrink the derived size)
   so every instance trails a constant gap; assert `VersionSkew`, not `Hole`.
5. **Compact archive ⇒ refused**, with the reason, not a gap list.

The fixture must contain a `contained[]` array (resource tuples) and a string array, or
REC-18.2's containment rule is untested — the same fixture-too-narrow trap that let 46
false `ExtentDerived` through for two review rounds.
