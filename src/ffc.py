...
# Target data types always generated by the library
TARGET_TYPES = [
...
def _annotate_code_enums(master_blocks, code_enum_map):
    """Tag layout entries whose code field has an enum from FF_CodeSystems."""
    ...
    for f in layout:
        ...
        if f['is_array']:
            if f['fhir_type'] in ('string', 'code'):
                ...
                if code_enum:
                    cpp += f"            STORE_FF_STRING(__base, write_head, std::string({code_enum['serialize']}({data_name}.{f['cpp_name']}[__i])));\n"
                else:
...
def generate_cxx_for_blocks(master_blocks, versions):
    ...
    for path, blk in master_blocks.items():
        ...
        for f in layout:
            ...
            elif code_enum: hpp += f"    {code_enum['enum']} {f['cpp_name']};\n"
...
def compile_fhir_library(resources, versions, input_dir="fhir_specs", output_dir="generated_src", code_enum_map=None):
    ...
    for t in target_types:
        ...
        if sch:
            ...
            h, c = generate_cxx_for_blocks(blocks, versions)
            hpp_head += f"// --- {t} ---\n{h}"
            cpp_head += f"// --- {t} ---\n{c}"
