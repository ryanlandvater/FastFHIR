...
namespace FastFHIR::Ingest
{
    ...
    struct ProducerCtx
    {
        ...
        void push_url(std::string_view url) noexcept
        {
            ...
            if (dedup_cache[slot] == h)
                return; // thread-local cache hit — skip
            dedup_cache[slot] = h;
    ...
    {
        if (is_faulted())
            return FF_Result{FF_FAILURE, "Engine is faulted. Reset() before ingesting new data."};
