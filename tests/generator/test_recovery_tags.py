...
`generated_src/FF_RecoveryTags.hpp` is generated by generator/emit/recovery_tags.py from
the committed ledger `dictionaries/master_tags.json`, and its values are
permanent wire constants -- every block on disk carries one at bytes 8-9. The
emitters *reference* those tags, but they build some of the names by
concatenation (`f"RECOVER_{child_struct}"` in generator/model/structure.py), so
a rename upstream can still produce a name the ledger never declared.
...
_RECOVERY = _REPO_ROOT / "generated_src" / "FF_RecoveryTags.hpp"
