...
def generate_eager_deserializer(layout, block_struct_name, data_name):
    ...
    for f in layout:
        ...
        if f.get('is_choice'):
            ...
            cpp += f"{indent}        Offset abs_off = FF_ResolveCustomStringOffset(raw_code, __offset);\n"
            cpp += f"{indent}        std::string_view custom_str = FF_STRING(abs_off, __size, __version).read_view(__base);\n"
            ...
            if code_enum: cpp += f"{indent}        data.{f['cpp_name']} = {code_enum['parse']}(std::string(FF_DECODE_CODEABLE_CONCEPT(__base, abs_off, __version)));\n"
            else:         cpp += f"{indent}        data.{f['cpp_name']} = FF_DECODE_CODEABLE_CONCEPT(__base, abs_off, __version);\n"
            cpp += f"{indent}    }}\n"
            cpp += f"{indent}}}\n"
