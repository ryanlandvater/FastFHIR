...
def generate_choice_block(production_types: list[str], auto_header: str) -> tuple[str, str]:
    ...
    hpp += (
        ...
        "\n"
        "// ChoiceEntry::operator=(T&&) -- declared in FF_Primitives.hpp, defined\n"
        "// here because it builds a ChoiceBlock, which only this header completes.\n"
        "template <typename T>\n"
        "    requires(!std::is_same_v<std::decay_t<T>, ChoiceEntry>)\n"
        "inline ChoiceEntry &ChoiceEntry::operator=(T &&incoming)\n"
        "{\n"
        "    using Value = std::decay_t<T>;\n"
        "    static_assert(std::is_constructible_v<decltype(ChoiceBlock::value), Value>,\n"
        '                  "this type is not one of the FHIR datatypes a choice slot can hold");\n'
        "    tag = TypeTraits<Value>::recovery;\n"
        "    block = std::make_unique<ChoiceBlock>(ChoiceBlock{std::forward<T>(incoming)});\n"
        "    value = std::monostate{};  // the payload is the block, not the inline arm\n"
        "    return *this;\n"
        "}\n"
    )
    ...
    cpp += (
        ...
        "// A value type: the copy deep-copies the block, so a POCO holding one\n"
        "// can be copied, and therefore brace-initialized.\n"
        ...
        "\n"
        "// The copy deep-copies the block: two ChoiceEntries must never share\n"
        "// one ChoiceBlock, or storing either would write the same child twice.\n"
        "ChoiceEntry::ChoiceEntry(const ChoiceEntry &other)\n"
        "    : tag(other.tag), value(other.value),\n"
        "      block(other.block ? std::make_unique<ChoiceBlock>(*other.block) : nullptr) {}\n"
        "\n"
        "ChoiceEntry &ChoiceEntry::operator=(const ChoiceEntry &other)\n"
        "{\n"
