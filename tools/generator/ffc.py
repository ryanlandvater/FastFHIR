...
# =====================================================================
# 6. PYTHON FIELD TOKEN EMITTER (for O(1) PyNode access in Python bindings)
# =====================================================================
def emit_python_fields(python_resource_map, output_dir="generated_src"):
    """
    Generates the pure Python fields.py file.
    Each field is a typed _core.Field(index, name, owner).
    """
    target_dir = os.path.join(output_dir, "python")
    os.makedirs(target_dir, exist_ok=True)
    file_path = os.path.join(target_dir, "fields.py")
    ...
    print(f"-- Emitted Python field tokens: {file_path}")
...
def compile_fhir_library(resources, versions, input_dir="fhir_specs", output_dir="generated_src", code_enum_map=None):
    ...
    for path, layout in sorted(block_key_defs, key=lambda item: item[0]):
        ...
        for f in layout:
            ...
            python_resource_map[class_name][f['orig_name']] = (token_id, f['orig_name'], ns_name)
            token_id += 1
    ...
    # Finally, emit the Python field tokens for bindings
