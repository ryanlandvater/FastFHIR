...
| Path | Role |
|---|---|
| `include/` | Public + internal headers. `FastFHIR.hpp` is the C++ consumer entry point **and the one place the global `FF_` type aliases are declared** (`FF_Builder = FastFHIR::Builder`, `FF_String`, `FF_Optional`, `FF_Memory`, ...) — see CONTRIBUTING absolute 4. `FastFHIR.h` is the **C ABI** (pure C, opaque handles, `src/FF_C_API.cpp` implements it; the shared facts are gated by `tests/generator/test_c_abi.py`) — a separate surface, not an include of the `.hpp`. `FF_Primitives.hpp` defines wire constants — **hand-maintained, values permanent**. `FF_String.hpp` is the POCO string member type; `FF_TypeTraits.hpp` holds the primitive `TypeTraits`; `FF_Export.h` is the shared `FF_EXPORT` visibility macro. The generated tags header is `generated_src/FF_RecoveryTags.hpp` (renamed 2026-08-27 from `FF_Recovery.hpp`); the recovery API lives in the public `include/FF_Recovery.hpp`. |
| `src/` | Core library: Memory (VMA), Builder, Parser, Compactor, Ingestor, Dictionary, Extensions (WASM), Primitives, and `FF_C_API.cpp` (the C ABI shim over the C++ API). |
...
1. **Wire constants are permanent.** Never renumber or reorder: `RECOVERY_TAG` values
   (`dictionaries/master_tags.json` — the committed tag ledger; `generated_src/FF_RecoveryTags.hpp` is
...
2. **Never hand-edit generated files** (`generated_src/` — which now includes `FF_Codes.hpp`
   ...
   **and committed** — they are permanent wire artifacts, reviewed in diffs.
   `generated_src/FF_RecoveryTags.hpp` is a *projection* of one of them and is **not** committed
